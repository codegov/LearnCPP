...
private:
    ...
    void testStructSize()
    {
        ...
        {
            int id;         //[0]....[3]
            double weight;  //[8]........[15]　原则１ 不是第一个数据，存储的起始位置要从自身大小的整数倍开始，double的大小是8字节，则从8字节开始存储
            float height;   //[16]....[19]
        }BB;                //大小到了20，原则３，总大小是其内部最大成员的整数倍，最大成员为double，大小为8，则补齐字节[20]...[23]，总大小为24.
        
        typedef struct aa
        {
            char name[2];   //[0].[1]
            int  id;        //[4]....[7]　　　　原则１ 不是第一个数据，存储的起始位置要从自身大小的整数倍开始，int的大小是4字节，则从4字节开始存储
            double score;   //[8]....[15]
            short grade;    //[16].[17]
            BB b;           //[24]......[47]　 原则２ 结构体成员要从其内部最大元素大小的整数倍地址开始存储
        }AA;                //大小到了48，是其内部最大成员BB(大小为22)的整数倍，不需要补齐，总大小为48.
        ...
        printf("内存字节对齐:%ld %ld\n", sizeof(BB), sizeof(AA));// 24 48
        printf("struct size:%ld %ld %ld\n", sizeof(SS), sizeof(SS.UU1), sizeof(SS.UU2));//16 2 8
    }
